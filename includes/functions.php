...
*//*
		...
		$query="SELECT DISTINCT(pd.personal_id), pd.lname, pd.fname, pd.mname, pd.name_ext, pd.bdate, pd.contact_no, pd.permanent_address, pd.provincial_address, pd.place_birth, pd.emp_status, pd.emp_num, pd.contact_no, pd.email, pd.nationality, pd.religion, pd.status, p.position_applied, p.date_applied, pd.supervisor, pd.status, pd.current_location,pd.date_hired FROM personal_data pd LEFT JOIN position p ON p.personal_id = pd.personal_id AND pd.supervisor='0'";
...
function getAmendmentSupInfo($conn, $column, $table, $id, $value_id){
	$get = $conn->query("SELECT $column FROM $table WHERE $id = '$value_id' ORDER BY $column DESC") or die(mysql_error());
	$fetch = $get->fetch_array();
	return $fetch[$column];
}
