...
<page size="A4">
    ...
    <?php 
        ...
        while($row=mysqli_fetch_array($sql)){
            ...
            $corporate_name = getInfo($con, 'bu_name', 'business_unit', 'bu_id', $buid);
            $department = getInfo($con, 'dept_name', 'department', 'dept_id', $rows['current_dept']);
            $current_supervisor='';
            $sup_designation='';
            if($row['current_sup'] != 0){
                $fname=getInfo($con, 'fname', 'personal_data', 'personal_id', $row['current_sup']);
                $lname=getInfo($con, 'lname', 'personal_data', 'personal_id', $row['current_sup']);
                $sup_designation=getCurrentJob($con, $row['current_sup'], $job_history);
            ...
            //     $sup_designation=getCurrentJob($con, $rows['current_supervisor'], $job_history);
            //     $current_supervisor=sanitize(utf8_encode($fname." ".$lname));
            // }
            // $fname=getInfo($con, 'fname', 'personal_data', 'personal_id', $rows['current_supervisor']);
            // $lname=getInfo($con, 'lname', 'personal_data', 'personal_id', $rows['current_supervisor']);
            // $sup_designation=getInfo($con, 'j_position', 'job_history', 'personal_id', $rows['current_supervisor']);
            // $current_supervisor=sanitize(utf8_encode($fname." ".$lname));
    ?>
    <?php include('../forms/logo_header_progen.php') ?>
    <table width="100%" class="nobor-top">
        ...
        <tr>
            ...
            <td class="bor-btm bor-right"><b><?php echo (!empty($rows['bdate'])) ? date("F d, Y",strtotime($rows['bdate'])) : ''; ?></b></td>
        ...
        <tr>
            ...
            <td class="bor-btm bor-right"><b><?php echo (!empty($date_reg)) ? date("F d, Y",strtotime($date_reg)) : ''; ?></b></td>
        </tr>
        <tr>
            ...
            <td class="nobor">
                ...
                <table width="100%" class="nobor-all">
                    ...
                    <tr>
                        ...
                        <td class="bor-top" align="center" style="font-size: 10px;"><?php echo strtoupper($sup_designation); ?></td>
                        <td class="nobor" width="10%"></td>
                        <td class="bor-top" align="center" style="font-size: 10px;">PRESIDENT & GENERAL MANAGER</td>
                        <td class="nobor" width="10%"></td>
                        <td class="bor-top" align="center" style="font-size: 10px;">EXECUTIVE DIRECTOR</td>
                    </tr>
                </table>
            </td>
        </tr>
