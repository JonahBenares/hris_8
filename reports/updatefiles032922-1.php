...
$$var = mysqli_real_escape_string($con, $value);
	...
	$count=1;
	while($fetch_cert = $check_cert->fetch_array()){
		$change_extension = explode(".", $fetch_cert['cert_file']); //certificate file
		$certname = 'cert_name'.$count;
		$cesname=$$certname;
		if (file_exists("../uploads/".utf8_decode($fetch_cert['cert_file']))) {
			$change_ext = explode(".", $fetch_cert['cert_file']); //certificate file
			$cext = $change_ext[1];
			$change_name = $lname."_".$fname."_".$cesname.$count.".".$cext;
	...
	if($rows_cert == $ctrx){
		...
		for($x=1; $x<=$ctrx;$x++){
			...
			if(!empty($_FILES[$c]["name"])){
				...
				$fetch_eval1 = $select1->fetch_array();
				if($row1>0){
					$change_extension = explode(".", $fetch_cert['cert_file']); //certificate file
					$cext1 = $change_extension[1];
					$cfile = $lname."_".$fname."_".$cename.$x.".".$cext1;
	...
	$count2=1;
	while($fetch_eval = $check_eval->fetch_array()){
		...
		if (file_exists("../uploads/".utf8_decode($fetch_eval['eval_file']))) {
			...
			$evext = $eval_ext[1];
			$change_evalname = $lname."_".$fname."_".$evalperiod.$count2.".".$evext;
	...
	$count3=1;
	while($fetch_other = $check_other->fetch_array()){
		...
		if (file_exists("../uploads/".utf8_decode($fetch_other['other_file']))) {
			$ot_ext = explode(".", $fetch_other['other_file']); //certificate file
			$otext = $ot_ext[1];
			$change_otname = $lname."_".$fname."_".$otname.$count3.".".$otext;
