<?php
	...
	if($amend_id==''){
		...
		mysqli_query($con,"INSERT INTO amendment (amendment_id,personal_id, date_prepared, date_effectivity, change_reason, remarks, dept_head,current_sup,draft,draft_date) VALUES('$amendment_id','$employee_name','$date_prepared','$date_effectivity','$change_reason','$remarks','$current_supid','$current_sup','1','$date_draft')");
		echo $amendment_id;
	}else{
		$count= count($_POST['change_name']);
		for($x=0; $x<$count;$x++){
		...
		$date_draft=date('Y-m-d');
		mysqli_query($con,"UPDATE amendment SET personal_id='$employee_name', date_prepared='$date_prepared', date_effectivity='$date_effectivity', change_reason='$change_reason', remarks='$remarks', dept_head='$current_supid', current_sup='$current_sup', draft='1', draft_date='$date_draft' WHERE amendment_id = '$amend_id'");
		echo $amend_id;
	}	
?>