<?php
	...
	$sup_designation=getInfo($con, 'j_position', 'job_history', 'personal_id', $row['current_supervisor']);
    ...
    $return = array('bday' => date("F d, Y",strtotime($row['bdate'])), 'civil_status' => $row['civil_status'], 'date_hired' => (!empty($fetch_personal['bdate'])) ? date("F d, Y",strtotime($fetch_personal['date_hired'])) : '', 'corporate_name' => $corporate_name, 'sss' => $sss, 'philhealth' => $philhealth, 'pagibig' => $pagibig,'tin' => $tin, 'designation' => $designation,'end_contract' => $end_contract, 'date_reg' => $date_reg, 'department' => $department, 'salary' => $salary, 'allowance' => $allowance, 'emp_status' => $row['emp_status'], 'current_supid' => $row['current_supervisor'], 'current_supervisor' => strtoupper($current_supervisor), 'sup_designation' => strtoupper($sup_designation)); 
