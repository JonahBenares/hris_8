<?php
	...
	if($amend_id==''){
		...
		$date_draft=date('Y-m-d');
		if($progen_flag==0){
			mysqli_query($con,"INSERT INTO amendment (amendment_id,personal_id, date_prepared, date_effectivity, change_reason, remarks, dept_head,current_sup,draft,draft_date,progen_flag) VALUES('$amendment_id','$ins_employee','$date_prepared','$date_effectivity','$change_reason','$remarks','$current_supid','$current_sup','1','$date_draft','0')");
		}else{
			mysqli_query($con,"INSERT INTO amendment (amendment_id,personal_id, date_prepared, date_effectivity, change_reason, remarks, dept_head,current_sup,draft,draft_date,progen_flag) VALUES('$amendment_id','$ins_employee','$date_prepared','$date_effectivity','$change_reason','$remarks','$current_supid','$current_sup','1','$date_draft','1')");
		}
		echo $amendment_id;
	}else{
		$count= count($_POST['change_name']);
		...
		$date_draft=date('Y-m-d');
		if($progen_flag==0){
			mysqli_query($con,"UPDATE amendment SET personal_id='$ins_employee', date_prepared='$date_prepared', date_effectivity='$date_effectivity', change_reason='$change_reason', remarks='$remarks', dept_head='$current_supid', current_sup='$current_sup', draft='1', draft_date='$date_draft',progen_flag='0' WHERE amendment_id = '$amend_id'");
		}else{
			mysqli_query($con,"UPDATE amendment SET personal_id='$ins_employee', date_prepared='$date_prepared', date_effectivity='$date_effectivity', change_reason='$change_reason', remarks='$remarks', dept_head='$current_supid', current_sup='$current_sup', draft='1', draft_date='$date_draft',progen_flag='1' WHERE amendment_id = '$amend_id'");
		}
		echo $amend_id;
	}	
?>